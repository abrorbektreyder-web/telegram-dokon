...
export default function CustomerHistory({ user }: { user: any }) {
  ...
  if (!user?.id) return (
    <div className="text-center py-20 opacity-30 px-10">
       ...
       <p className="font-black text-[10px] uppercase tracking-widest leading-relaxed">{t('login_required')}</p>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-2 px-2">
        <h3 className="font-black text-[12px] uppercase tracking-[0.2em] text-slate-400">{t('my_history')}</h3>
        <button onClick={fetchHistory} className="text-sky-500 text-[10px] font-bold">Refresh</button>
      </div>

      {history.length === 0 ? (
        <div className="bg-slate-50 border-2 border-dashed border-slate-100 rounded-[2rem] p-10 text-center">
           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('no_history')}</p>
        </div>
      ) : (
        history.map((item) => (
          <motion.div 
          ...
          >
            <div className="flex items-center gap-4">
               ...
               <div>
                  <h4 className="font-black text-sm text-slate-800">{item.service_name || 'Purchase'}</h4>
